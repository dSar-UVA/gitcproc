...
#--------------------------------------------------------------------------------------------------------------------------
def szz(project_corpus_path, 
        project_snapshots_path, 
        bugfix_SHAs_filename, num_of_cores = '4',
        ps_bug_report_times_filename = ''):
    
    """
        Computes the SHAs where all the fix-inducing lines were introduced (along the lines of SZZ) 
        and records the precise location of each such line in a CSV file in the `project_corpus_path` directory.
    ...
    """
    print (',').join((project_corpus_path,project_snapshots_path,bugfix_SHAs_filename))
    ...
    print("Working with these input arguments %s,%s,%s " % (project_corpus_path, project_snapshots_path, bugfix_SHAs_filename))

    project_corpus_path += os.sep
    project_snapshots_path += os.sep
    ...
    ss_names = [ss_name for ss_name in os.listdir(project_snapshots_path) \
                if os.path.isdir(project_snapshots_path + os.sep + ss_name) \
                   and os.path.isdir(project_corpus_path + os.sep + ss_name)]
    
    if ss_names == []:
        raise ValueError("\nNo valid snapshots found in `project_corpus_path`")
    ss_names.sort()

    ss_paths = [project_snapshots_path + os.sep + ss_name + os.sep for ss_name in ss_names]
    ss_changes_paths = [project_corpus_path + os.sep + ss_name + os.sep for ss_name in ss_names]
    ...
    for ss_index, ss_changes_path in enumerate(ss_changes_paths):
        process_ss_cmd = "python map_bugs2snapshot/szz_process_ss.py " + ss_names[ss_index] + " " + ss_paths[ss_index] + " " \
                          + ss_changes_path + " " + bugfix_SHAs_filename
        #print process_ss_cmd
        #print Util.runCmd(process_ss_cmd)
        cmds.append(shlex.split(process_ss_cmd))
        #szz_process_ss(ss_names[ss_index], ss_paths[ss_index], ss_changes_path, bugfix_SHAs_filename)
    ...
    for ss_index, ss_changes_path in enumerate(ss_changes_paths):
        ss_bugdata_outfile_name = ss_changes_path + os.sep + 'ss_mappedOntoSSOnly.bugdata'
        try:
    ...
    bugdata_csv_filename = project_corpus_path + os.sep + 'ss_bugdata.csv'
