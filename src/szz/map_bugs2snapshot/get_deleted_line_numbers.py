#--------------------------------------------------------------------------------------------------------------------------
import os, sys, shlex, subprocess, csv
...
if __name__ == "__main__":
    if len(sys.argv) != 4 or not os.path.isdir(sys.argv[2]) or not os.path.isfile(sys.argv[3]):
        print("\nUsage: python get_deleted_line_numbers.py <data_dir> <project_name> <path_to_showlinenum_script>")
        print("\nSample usage: python get_deleted_line_numbers.py data/ libuv bin/showlinenum.awk\n")
    ...
    bf_sha_filename = data_dir + "bf_shas/" + project_name

    # Getting the list of bugfix-shas for this project
    bf_shas = []
    with open(bf_sha_filename, "r") as bf_sha_file:
        bf_shas = bf_sha_file.read().splitlines()
    if len(bf_shas) == 0:
        raise ValueError("No SHAs of bugfix commits were found! I checked this file: " + bf_sha_filename)
    else:
    ...
    out_file_name = out_dir + project_name + ".buggylines"
